...
const encryptionPW = (pw) => {
  ...
  const jsePublicKey = window.vm.$store.getters.getRsaPublicKey
...
const getRemembePW = () => {
  if (Cookies.get('hn') && Cookies.get('hp')) {
    const username = Cookies.get('hn')
    const mpassword = Cookies.get('hp')
    const password = reverseString(mpassword).split(mixStr)[0]
    return {
      username,
      password
    }
...
const combineNode = (tree, node) => {
  const currentleaf = node.leaf
  const leafArr = currentleaf.split('-')
  if (leafArr.length === 1) {
    tree[leafArr[0]] = node
  }
  if (leafArr.length === 2) {
...
const formatAuth = (data, authList = []) => {
  data.forEach((item, index) => {
    ...
    if (item.children.length) {
      ...
      // 当按钮权限的时候，进行排序；访问权限在前
      if (item.authType === '2' && item.actionType) authList.sort((a, b) => a.meta.actionType - b.meta.actionType)
    }
  })
}
...
const getMenuNameArr = (routerName) => {
  const nameArr = routerName.split('_')
  const routerNameArr = []
  ...
  if (redirectedFromPath && menuTree) {
    for (let i = 0; i < menuTree.length; i++) {
      if (redirectedFromPath === menuTree[i].path) {
        const firstChildMenu = getFirstLinkMenu(menuTree[i])
